...
function countVowels(str) {
  ...
  let res = 0;
  for (let i = 0; i < str.length; i += 1) if (vowels.includes(str[i])) res += 1;
  return res;
}
...
function isPalindrome(str) {
  ...
  return (
    ...
    str.toLowerCase().replaceAll(regex, '').split('').reverse().join('')
  );
}
...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i < arrayOfWords.length; i += 1)
    if (arrayOfWords[i].length > res.length) res = arrayOfWords[i];
  return res;
}
...
function invertCase(str) {
  ...
  for (let i = 0; i < arrayOfLetters.length; i += 1)
    if (arrayOfLetters[i].match(/[a-z]/))
      arrayOfLetters[i] = arrayOfLetters[i].toUpperCase();
    else arrayOfLetters[i] = arrayOfLetters[i].toLowerCase();
...
function encodeToRot13(str) {
  const templateFrom = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  const templateTo = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  const arrayOfLetters = str.split('');
  for (let i = 0; i < arrayOfLetters.length; i += 1)
    if (templateFrom.includes(arrayOfLetters[i]))
      arrayOfLetters[i] = templateTo[templateFrom.indexOf(arrayOfLetters[i])];
  return arrayOfLetters.join('');
}
